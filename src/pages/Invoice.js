  ...
  if (value.startsWith("0") && value.length > 1 && !value.startsWith("0.")) return;
  ...
  // ✅ لو داخل تعديل فاتورة لا تحمل Draft
  if (id) return;
...
useEffect(() => {
  ...
  // ✅ لا تحفظ Draft أثناء تعديل فاتورة
  if (id) return;
  ...
  c => Number(c.customerNumber) === Number(inv.customerCode)
);

if (!customer) {
  console.log("❌ العميل مش موجود:", inv.customerCode);
}
...
setSelectedCustomer(customer || null);
  ...
  if (index !== -1) {
    ...
    console.log("❌ الفاتورة مش موجودة:", id, invoices);
  }
  ...
  invoiceItems
   ...
   if (!selectedCustomer || !selectedWarehouse || invoiceItems.length === 0) {
  alert("Please complete invoice data");
  return;
}
...
/* 🧹 لو تعديل فاتورة → امسح القديم */
if (currentIndex !== -1) {
  const oldInvoice = invoices[currentIndex];

  cashTransactions = cashTransactions.filter(
    (t) =>
      String(t.invoiceNumber) !== String(oldInvoice.invoiceId)
  );
}
...
if (paymentMethod === "cash") {
  ...
  if (bankCash.length) {
    bankCash[0].balance += Number(netTotal);
    localStorage.setItem("bankCash", JSON.stringify(bankCash));
  }
...
// ✅ تنظيف القيم قبل الحفظ (مهم جدًا)
let finalPaid = 0;
...
if (paymentMethod === "credit") {
  finalPaid = Number(paidAmount || 0);
  finalRemaining = netTotal - finalPaid;
}

if (paymentMethod === "cheque") {
  finalPaid = Number(chequeAmount || 0);
  finalRemaining = netTotal - finalPaid;
}
if (paymentMethod === "credit" && Number(paidAmount) > 0) {
  ...
  cashTransactions.push({
    ...
    description: "تحصيل من فاتورة آجل",
    date: invoiceDate
  });
...
remainingAmount: finalRemaining,
    ...
    currentIndex !== -1
      ...
      : nextInvoiceNumber,
  customerName: selectedCustomer.name,
customerCode: Number(selectedCustomer.customerNumber),
      warehouse: selectedWarehouse,
      items: invoiceItems,
      date: invoiceDate,
      time: invoiceTime,
...
/* ===== 1️⃣ رجوع الرصيد القديم ===== */
if (currentIndex !== -1) {
  const oldInvoice = invoices[currentIndex];

  if (oldInvoice.paymentMethod === "credit") {
    updatedCustomers = updatedCustomers.map(c => {
      if (Number(c.customerNumber) === Number(oldInvoice.customerCode)) {
        return {
          ...c,
          balance:
            Number(c.balance || 0) -
...
/* ===== 2️⃣ إضافة الرصيد الجديد ===== */
if (paymentMethod === "credit") {

  const remaining =
    netTotal - Number(paidAmount || chequeAmount || 0);

  if (remaining > 0) {
    updatedCustomers = updatedCustomers.map(c => {
      if (Number(c.customerNumber) === Number(selectedCustomer.customerNumber)) {
        return {
          ...c,
          balance:
            Number(c.balance || 0) + remaining
        };
      }
      return c;
    });
...
const collectCheque = (invoiceId) => {
  ...
  invoices = invoices.map(inv => {
    ...
    if (String(inv.invoiceId) === String(invoiceId)) {

      cashTransactions.push({
        id: Date.now(),
        type: "in",
        operationType: "cheque-collection",
        customerName: inv.customerName,
        customerCode: inv.customerCode,
        amount: Number(inv.remainingAmount || 0),
        date: today,
        description: "تحصيل شيك - فاتورة " + inv.invoiceId
      });
  ...
  let updatedCustomers = [...customers]; // 👈 مهم copy

  /* 1️⃣ لو تعديل فاتورة → رجّع القديم */
 if (currentIndex !== -1) {
  const oldInvoice = invoices[currentIndex];
  ...
  /* ===== حذف حركة المخزون الخاصة بالفاتورة ===== */

let movements =
JSON.parse(localStorage.getItem("stockMovements")) || [];

movements = movements.filter(
(m) => String(m.reference) !== String(invoiceToDelete.invoiceId)
);
...
localStorage.setItem("stockMovements", JSON.stringify(movements));
 ...
 currentIndex !== -1 &&
 !invoices[currentIndex]?.collected && (

  <button
    className="btn btn-warning"
    onClick={() => collectCheque(displayedInvoiceNumber)}
  >
    💰 تحصيل الشيك
  </button>
...
onChange={(e) => handleNumberInput(e.target.value, setQty)}
  />
      </div>
      ...
      {paymentMethod === "credit" && (
        <div className="form-row">
          ...
          <input
            ...
            className="erp-input small"
            value={paidAmount}
          onChange={(e) => handleNumberInput(e.target.value, setPaidAmount)}
          />
        </div>
      )}
          ...
          <div className="form-row">
            ...
            <input
             ...
             inputMode="numeric"
              className="erp-input small"
              value={chequeAmount}
             onChange={(e) => handleNumberInput(e.target.value, setChequeAmount)}
            />
          </div>
